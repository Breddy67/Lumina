...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (float y = 0.0f; y < (GetScreenHeight() / step); y += 1) {
            for (float x = 0.0f; x < (GetScreenWidth() / step); x += 1) {
                float px = (x * step);
                float py = (y * step);
                float n = lumen_noise2((px * scale), (py * scale));
                if ((n < (-0.5f))) {
                    DrawRectangle((int)(((Vector2){ px, py }).x), (int)(((Vector2){ px, py }).y), (int)step, (int)step, (Color){ 0, 0, 255, 255 });
                } else {
                    if ((n < (-0.2f))) {
                        DrawRectangle((int)(((Vector2){ px, py }).x), (int)(((Vector2){ px, py }).y), (int)step, (int)step, (Color){ 0, 136, 255, 255 });
                    } else {
                        if ((n < 0.0f)) {
                            DrawRectangle((int)(((Vector2){ px, py }).x), (int)(((Vector2){ px, py }).y), (int)step, (int)step, (Color){ 0, 255, 136, 255 });
                        } else {
                            if ((n < 0.2f)) {
                                DrawRectangle((int)(((Vector2){ px, py }).x), (int)(((Vector2){ px, py }).y), (int)step, (int)step, (Color){ 0, 255, 0, 255 });
                            } else {
                                if ((n < 0.5f)) {
                                    DrawRectangle((int)(((Vector2){ px, py }).x), (int)(((Vector2){ px, py }).y), (int)step, (int)step, (Color){ 255, 136, 0, 255 });
                                } else {
                                    DrawRectangle((int)(((Vector2){ px, py }).x), (int)(((Vector2){ px, py }).y), (int)step, (int)step, (Color){ 255, 0, 0, 255 });
                                }
                            }
                        }
                    }
