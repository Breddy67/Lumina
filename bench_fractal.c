...
void branch(float px, float py, float ang, float len, float depth) {
    float rad = (ang * (PI / 180.0f));
    float end_x = (px + (cosf(rad) * len));
    float end_y = (py + (sinf(rad) * len));
    DrawLineV((Vector2){ px, py }, (Vector2){ end_x, end_y }, (Color){ 255, 255, 255, 255 });
    if ((depth > 0.0f)) {
        branch(end_x, end_y, (ang - 20.0f), (len * 0.7f), (depth - 1.0f));
        branch(end_x, end_y, (ang + 20.0f), (len * 0.7f), (depth - 1.0f));
    }
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        branch(200.0f, 380.0f, (-90.0f), 80.0f, 6.0f);
        EndDrawing();
    }
